const search_Dense_data = (func, arch, precision, size, version, y_axis) => {
    let data = [];
...
const plot_Dense_all = (arch, prec, size, y_axis, version) =>{
    ...
    let dataX = kind_Dense_list.sort();

    let dataY = [];
    // create plot data
    for(let i=0; i< dataX.length; i++){
        dataY.push(search_Dense_data(dataX[i], arch, prec, size, version, y_axis));
    }
...
const PlotBtn4 = () => {
    ...
    plot_Dense_all(arch, prec, size, yaxis, version);

};

const create_choice4 = () => {
    ...
    if(document.getElementById("choice_area4") == null){
        ...
        text += '<br><input type="button" value="Plot!" onclick="PlotBtn4()"/>'
